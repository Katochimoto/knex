...
class Client_SQLJS extends Client {
  constructor(config) {
    ...
    if (config.connection && config.connection.bytes === undefined) {
      this.logger.warn(
        'Could not find `connection.bytes` in config. Please specify ' +
          'the database initial data in Uint8Array to avoid errors. '
      );
    }
  ...
  _driver() {
    return require('sql.js/dist/sql-asm');
  }
  ...
  acquireRawConnection() {
    return new Promise((resolve, reject) => {
      this.driver().then((SQL) => {
        const db = new SQL.Database(this.connectionSettings.bytes || Uint8Array.from([]));
        resolve(db);
      }).catch(reject);
    });
  }
